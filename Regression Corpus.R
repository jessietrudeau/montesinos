# ──────────────────────────────────────────────────────────────────────────────
# Full R script: Read transcripts, tag with topic & actor type, clean text, build DFMs
# ──────────────────────────────────────────────────────────────────────────────
...
# 2. Read your inventory and build a topic lookup table
inventory <- read_csv(
...
# 6. Read all transcripts, combine, and join topic & actor info
text_data <- file_list %>%
  lapply(read_text_data) %>%
  bind_rows() %>%
  left_join(topic_lookup, by = "n") %>%
  left_join(actor_lookup, by = "speaker_std")

# 7. Sanity checks
if (!"speech"      %in% colnames(text_data)) stop("ERROR: Column 'speech' not found.")
if (!"topic"       %in% colnames(text_data)) stop("ERROR: Column 'topic' not found.")
if (!"Type"        %in% colnames(text_data)) stop("ERROR: Column 'type' not found.")
...
# 9. Build a Quanteda corpus (doc‐vars now include file, n, topic, type, speaker, speaker_std, etc.)
...
# ──────────────────────────────────────────────────────────────────────────────
# Full R script: Read transcripts, tag with topic, date & actor type, clean text, build DFMs
# ──────────────────────────────────────────────────────────────────────────────
